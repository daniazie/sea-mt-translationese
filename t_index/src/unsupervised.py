...
def main(args):
    tokenizer = AutoTokenizer.from_pretrained('/'.join(args.model_positive.split('/')[:-1]))
    ...
    if vars(args).get('validation_path'):
    ...
    for path in glob(args.data_path):
        ...
        for i in tqdm(range(0, len(dataset_for_positive_model), args.batch_size), desc="Evaluating"):
            ...
            results["file_path"].extend([path] * len(log_lklh_positive.tolist()))
