...
def format_dataset_with_system_prompt(examples, src_lang, tgt_lang):
    system_prompt = """You are an expert editor. Your task is to edit texts translated into the {tgt_lang} language, with the goal of making the translation awkward and unnatural. In other words, edit the translation to be obviously 'translationese' based on the provided source text.
    Your edited translation should:
\t- Be overly literal and overly faithful to the source text
\t- Lack nuances of idiomatic {tgt_lang}
\t- Reflect conventions of the original source language instead of that of the {tgt_lang} language
\t- Have 'technically correct' syntax whilst still being unnatural to a native speaker
\t- Retain the factual and semantic accuracy of both the source text and translation.
\t- Be highly distinguishable from {tgt_lang} texts written by native speakers.
    
    Return ONLY the edited translation, without commentary.
""".format(tgt_lang=lang_code_to_name[tgt_lang])
    user_prompts = [f"""Source text: {src}\n\nTranslation: {ref}""".strip() for src, ref in zip(examples['src'], examples['ref'])]
    if args.is_vl:
...
def format_dataset_user_prompt_only(examples, src_lang, tgt_lang):
    instruction = """You are an expert editor. Your task is to edit texts translated into the {tgt_lang} language, with the goal of making the translation awkward and unnatural. In other words, edit the translation to be obviously 'translationese' based on the provided source text.
    Your edited translation should:
\t- Be overly literal including with idiomatic expressions
\t- Ignore conventions of the {tgt_lang} language, and instead reflect the linguistic style of the source text
\t- Lack nuances of idiomatic {tgt_lang}
\t- Have 'technically correct' syntax whilst still being unnatural to a native speaker
\t- Retain the factual and semantic accuracy of both the source text and translation.
\t- Be highly distinguishable from {tgt_lang} texts written by native speakers (including the provided translation).
    
    Return ONLY the edited translation, without commentary.
""".format(tgt_lang=lang_code_to_name[tgt_lang])
    
    prompts = [instruction + f"""Source text: {src}\n\nTranslation: {ref}""".strip() for src, ref in zip(examples['src'], examples['ref'])]
    messages = []
...
def generate_data(split: str | None = None, dataset: DatasetDict | Dataset | None = None):
    preds = []
    if not args.wild:
    ...
    if args.wild:
        ...
        result_file = f'{args.output_dir}/{split}.jsonl'
