from transformers import AutoModelForCausalLM, AutoTokenizer, Trainer, TrainingArguments, LlamaForCausalLM, HfArgumentParser, set_seed
from peft import LoraConfig, BOFTConfig, AdaLoraConfig, HRAConfig, EvaConfig, PeftModel, get_peft_model
...
from trainer import TEvalTrainer, TEvalTrainingArguments
...
if __name__ == "__main__":
    ...
    if use_peft:
        ...
        if "lora" in args.peft_config and isinstance(args.init_lora_weights, SyntaxError):
    ...
    for data_file in data_files:
        if "cleaned" in data_file and not 'test' in data_file:
    ...
    tokenized_ds = prepare_dataset(dataset, args.instruction_prompt, tokenizer, convert_to_chat_template=False)
    ...
    if args.use_custom_opt_params:
        optimizer_cls_and_kwargs = (optim.AdamW, {"params": list(model.ht_model.named_parameters()) + list(model.lt_model.named_parameters())})
    else:
    ...
    save_model(trainer.model, f"{targs.output_dir}/final/model.safetensors", metadata={"format": "pt"})
    model.tokenizer.save_pretrained(f"{targs.output_dir}/final", safe_serialization=True)